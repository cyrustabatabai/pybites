...
def get_movies():
    ...
    return [movie.get('title') for movie in get_tree().findall('movie')]


def _get_runtime(movie_element):


    return int(movie_element.get('runtime').split()[0])
...
def get_movie_longest_runtime():
    ...
    return max((_get_runtime(movie),movie.get('title')) for movie in get_tree().findall('movie'))[1]
